...
string action[] = {"ADD", "MUL", "SUB", "DIV", "MOV", "CMP", "JE", "JNE", "JGE", "JLE", "PRN", "exit"};
...
void sejda(int cur) {
	...
	if (first[cur] == "JE") JE(cur);
	if (first[cur] == "JNE") JNE(cur);
	if (first[cur] == "JGE") JGE(cur);
	if (first[cur] == "JLE") JLE(cur);	
	*/
}
...
void startTheProgram() {
	...
	while (true) {
		printf ("in line %d\n", cur + 1);
		if (cur >= dataL || first[cur] == "exit") {
			printAll();
			exit(0);
		}
		if (actB[cur]) sejda(cur);
		cur++;
	}
}
